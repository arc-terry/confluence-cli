...
function createOperation(definition) {
  return Object.freeze({ maxOutputBytes: 48 * 1024, ...definition });
}
...
const OPERATIONS = Object.freeze(Object.assign(Object.create(null), {
  ...
  confluence_space_lookup: createOperation({
    toolName: 'confluence_space_lookup',
    maxOutputBytes: 16 * 1024,
    cliCommand: 'space-lookup',
    risk: RISK.READ,
    timeoutMs: 30_000,
    mutation: false,
    expectJson: true,
    buildArgs(params = {}) {
      return ['--json', 'space-lookup', requireText(params.spaceKey, 'spaceKey')];
    },
  }),
  ...
  confluence_comment_lookup: createOperation({
    toolName: 'confluence_comment_lookup',
    maxOutputBytes: 16 * 1024,
    cliCommand: 'comment-lookup',
    risk: RISK.READ,
    timeoutMs: 30_000,
    mutation: false,
    expectJson: true,
    buildArgs(params = {}) {
      return ['--json', 'comment-lookup', requireText(params.commentId, 'commentId')];
    },
  }),
  ...
  confluence_attachment_lookup: createOperation({
    toolName: 'confluence_attachment_lookup',
    maxOutputBytes: 16 * 1024,
    cliCommand: 'attachment-lookup',
    risk: RISK.READ,
    timeoutMs: 30_000,
    mutation: false,
    expectJson: true,
    buildArgs(params = {}) {
      return ['--json', 'attachment-lookup', requireText(params.attachmentId, 'attachmentId')];
    },
  }),
...
function getOperation(name) {
  const legacyName = CANONICAL_TO_LEGACY[name] ?? name;
  if (!Object.prototype.hasOwnProperty.call(OPERATIONS, legacyName)) {
    throw new Error(`Confluence operation "${name}" is not allowed.`);
  }
  return OPERATIONS[legacyName];
}

function listToolNames({ includeWrites = false } = {}) {
  const legacyNames = includeWrites ? TOOL_NAMES : READ_TOOL_NAMES;
  return Object.freeze(legacyNames.map((legacyName) => LEGACY_TO_CANONICAL[legacyName]).filter(Boolean));
}
