...
Set `CONFLUENCE_PI_BULK_ACTIONS=true` to additionally register `confluence_pages_batch` and `confluence_comments_batch`. Each batch preflights every action and requests one Pi UI confirmation before executing actions in order:
