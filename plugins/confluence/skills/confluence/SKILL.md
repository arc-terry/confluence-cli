...
Use canonical Pi resource-action names, including `confluence_page_read`, `confluence_pages_search`, `confluence_page_info`, `confluence_page_create`, `confluence_page_update`, `confluence_page_delete`, and `confluence_page_comment_create`. This is a breaking Pi-extension API change; standalone CLI command names are unchanged.

Protected mutation tools are registered only when the operator starts Pi with writes enabled and an allowed-space list. Set `CONFLUENCE_PI_BULK_ACTIONS=true` to also register `confluence_pages_batch` and `confluence_comments_batch`. Page batches support `create`, `create-child`, `update`, `move`, and `delete`; comment batches support `create`. Both preflight every action. In the Pi TUI, every action starts selected: Up/Down moves, Space toggles the highlighted action, `a` selects or clears all, Enter reviews, and Escape cancels. The final confirmation must exactly match `MANIPULATE <selected-count> ACTIONS: <selected-canonical-scope-tokens>`; selection does not authorize writes. Empty selection, cancellation, blank or mismatched confirmation, or abort starts no mutation. Only selected actions execute in input order and unselected actions are returned under `skipped`. RPC confirms the full batch with its existing exact confirmation because custom TUI selection is unavailable.
